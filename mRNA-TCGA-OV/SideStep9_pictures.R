...
res_coef_cox_names <- res_coef_cox_names[-39] #pasalinu grupe
...
age_genes7 <- c( "RAD50", "TTC4", "RPS10", "CLDN4", "ARPC1B", "PKP3", "GRB7",
...
ggplot(data=age_df,  aes(x = age_diagnosis_yrs, y = EXPR, color= stage))+
  geom_point() +
  facet_wrap(~Genes)
age_genes38 <- c( res_coef_cox_names,
                 "age_diagnosis_yrs", "stage")
age_df <- tcga %>% select(age_genes38 ) %>%
  na.omit() %>%
  pivot_longer(., cols = c(stage_genes38[1:38]), names_to = "Genes", values_to = "EXPR") %>%
  as.data.frame()
age_df$Genes <- as.factor(age_df$Genes)
...
resdis_genes7 <- c( "RAD50", "TTC4", "RPS10", "CLDN4", "ARPC1B", "PKP3", "GRB7",
                   "tumorresidualdisease") #i need to add group so that it will be sorted
tcga %>% select(resdis_genes7) %>%
  na.omit() %>%
  pivot_longer(., cols = c(stage_genes7[1:7]), names_to = "Genes", values_to = "EXPR") %>%
  ggplot(aes(x = Genes, y = EXPR, fill = tumorresidualdisease)) +
  geom_boxplot()+
...
hist(tcga$intermediate_dimension) #nelabai galiu grupem skirstyt, nebent gal +-60?
dim_genes7 <- c( "RAD50", "TTC4", "RPS10", "CLDN4", "ARPC1B", "PKP3", "GRB7",
                 "intermediate_dimension", "stage")
dim_df <- tcga %>% select(dim_genes7) %>%
  na.omit() %>%
  pivot_longer(., cols = c(dim_genes7[1:7]), names_to = "Genes", values_to = "EXPR") %>%
  as.data.frame()
dim_df$Genes <- as.factor(dim_df$Genes)
ggplot(data=dim_df,  aes(x = intermediate_dimension, y = EXPR, color= stage))+
  geom_point() +
  facet_wrap(~Genes)
dim_genes38 <- c( res_coef_cox_names,
                  "intermediate_dimension", "stage")
dim_df <- tcga %>% select(dim_genes38) %>%
  na.omit() %>%
  pivot_longer(., cols = c(stage_genes38[1:38]), names_to = "Genes", values_to = "EXPR") %>%
  as.data.frame()
dim_df$Genes <- as.factor(dim_df$Genes)
